...
// The dealer can't recall the information for a car with an id of 33 on his lot. Help the dealer find out which car has an id of 33 by logging the car's year, make, and model in the console log provided to you below:
 for (let car33 = 0; car33 < inventory.length; car33++) {
     if (inventory[car33].id === 33) {
         console.log(`car ${inventory[car33].id} is a ${inventory[car33].car_year} ${inventory[car33].car_make} ${inventory[car33].car_model}`);
...
let carModel = inventory.map((name) => {
    return{'car_model': name.car_model};
});
...
let carYear = inventory.map((year) => {
  return{'car_year': year.car_year};
});
console.log(carYear);
...
let oldCars = inventory.filter((old) => {
